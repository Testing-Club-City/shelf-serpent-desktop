...
def migrate_staff_borrowings():
    ...
    try:
        # Get staff mapping (phone number to staff_id)
        staff_map = {}
        ...
        print(f"Found {len(staff_map)} staff members in new database")
        
        # Get book mapping (legacy_book_id to book_id)
        book_map = {}
        book_cursor = new_conn.execute("""
            SELECT DISTINCT b.id, bc.legacy_book_id 
            FROM books b 
            JOIN book_copies bc ON b.isbn = bc.isbn 
            WHERE bc.legacy_book_id IS NOT NULL
        """)
        for book_id, legacy_book_id in book_cursor:
            book_map[legacy_book_id] = book_id
        
        print(f"Found {len(book_map)} books with legacy mapping")
        
        # Migrate active borrowings (IssueDetails)
        active_query = """
        ...
        for row in legacy_conn.execute(active_query):
            ...
            if phone in staff_map and book_id_legacy in book_map:
                borrowing_id = str(uuid.uuid4())
                staff_id = staff_map[phone]
                book_id = book_map[book_id_legacy]
                
                # Insert or update borrowing
                new_conn.execute("""
                    INSERT OR REPLACE INTO borrowings (
                        id, staff_id, book_id, borrowed_date, due_date, 
                        status, borrower_type, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, 'active', 'staff', ?, ?)
                """, (borrowing_id, staff_id, book_id, issue_date, due_date, 
                     datetime.now().isoformat(), datetime.now().isoformat()))
                
                active_count += 1
        
        # Migrate returned borrowings (SubmittedBooks)
        returned_query = """
        ...
        for row in legacy_conn.execute(returned_query):
            ...
            if phone in staff_map and book_id_legacy in book_map:
                ...
                """, (borrowing_id, staff_id, book_id, issue_date, due_date, submit_date,
        ...
        print(f"- Active staff borrowings: {active_count}")
        print(f"- Returned staff borrowings: {returned_count}")
        print(f"- Total staff borrowings migrated: {active_count + returned_count}")
