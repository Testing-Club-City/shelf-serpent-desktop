...
impl SyncEngine {
    ...
    pub async fn check_connectivity(&self) -> bool {
        self.check_connectivity_with_cache(false).await
    }

    pub async fn check_connectivity_cached(&self) -> bool {
        self.check_connectivity_with_cache(true).await
    }

    async fn check_connectivity_with_cache(&self, use_cache: bool) -> bool {
        // Check cache first if requested
        if use_cache {
            let status = self.status.read().await;
            let last_check = status.last_connectivity_check;
            if let Some(last_check) = last_check {
                let cache_duration = Duration::from_secs(15); // 15 second cache
                if last_check.elapsed() < cache_duration {
                    debug!("Using cached connectivity result: {}", status.is_online);
                    return status.is_online;
                }
            }
        }

        // Use a more reliable and faster endpoint - Cloudflare's 1.1.1.1
        let simple_check = self.client
            .get("https://1.1.1.1/")
            .timeout(Duration::from_secs(3)) // Reduced timeout
            .send()
            .await;
        ...
        if simple_check.is_ok() {
            ...
            info!("Internet connectivity confirmed via Cloudflare DNS");
            return true;
        }
        
        // Fallback to Google's public DNS (more reliable than httpbin)
        let fallback_check = self.client
            .get("https://dns.google/")
            .timeout(Duration::from_secs(2)) // Even shorter timeout for fallback
            .send()
            .await;
    ...
    pub async fn check_supabase_connection_cached(&self) -> bool {
        ...
        let last_check = status.last_connectivity_check;
        if let Some(last_check) = last_check {
            let cache_duration = Duration::from_secs(30); // 30 second cache for DB checks
            if last_check.elapsed() < cache_duration {
                debug!("Using cached Supabase connectivity result");
                return status.is_online; // Assume DB follows general connectivity
            }
        }
        drop(status);

        // Only check Supabase if we have a real URL (not placeholder)
        if !self.config.url.contains("your-project") && !self.config.url.is_empty() {
            ...
            {
                Ok(response) => {
                    let is_connected = response.status().is_success() || response.status() == 400; // 400 is OK for auth
                    info!("Supabase connectivity check result: {}", is_connected);
                    return is_connected;
                }
                Err(e) => {
