...
export const OfflineAuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const checkOfflineSession = async (skipOnlineAuth: boolean = false): Promise<boolean> => {
    try {
      ...
      // Tauri environment - use invoke calls with fallback
      let cachedSession: UserSession | null = null;
      
      try {
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Timeout')), 2000); // Reduced timeout
        });
        ...
        cachedSession = result as UserSession | null;
      } catch (invokeError) {
        console.warn('Failed to get cached session via Tauri, using fallback:', invokeError);
        // Fallback to localStorage even in Tauri environment
        const fallbackSession = localStorage.getItem('offline_session');
        if (fallbackSession) {
          try {
            const sessionData = JSON.parse(fallbackSession);
            cachedSession = sessionData as UserSession;
  ...
  // Ultra-fast auth initialization - bypass all delays
  useEffect(() => {
    console.log('⚡ Ultra-fast auth bypass - loading immediately');
    
    // Immediate auth bypass for ultra-fast loading
    const quickInit = async () => {
      try {
        // Try to get cached session super quickly (100ms max)
        const hasCache = await Promise.race([
          checkOfflineSession(true),
          new Promise<boolean>((resolve) => setTimeout(() => resolve(false), 100))
        ]);
        
        if (hasCache) {
          console.log('⚡ Found cached session instantly');
        } else {
          console.log('⚡ No cache - proceeding without auth');
          setUser(null);
        }
      } catch (error) {
        console.log('⚡ Quick auth failed, proceeding anyway');
        setUser(null);
      } finally {
        setLoading(false); // Always stop loading after 100ms max
      }
    };
  ...
  useEffect(() => {
    const safetyTimeout = setTimeout(() => {
      if (loading) {
        console.log('⛑️ Safety timeout triggered - forcing loading=false');
        setLoading(false);
      }
    }, 2000); // Reduced to 2 second absolute maximum for better UX
