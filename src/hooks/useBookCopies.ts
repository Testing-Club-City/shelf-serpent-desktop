...
export const useCreateBookCopies = () => {
  ...
  return useMutation({
    mutationFn: async ({ 
      ...
      console.log('Creating book copies using offline-first approach...');
      
      try {
        // Use Tauri command for offline-first functionality
        const { invoke } = await import('@tauri-apps/api/core');
        
        const copyIds = await invoke('add_book_copies', {
          bookId,
          totalCopies,
          startingCopyNumber,
          condition,
        ...
        console.log('Successfully created book copies:', copyIds);
        ...
        createLog.mutate({
          ...
          details: {
            ...
            starting_copy_number: startingCopyNumber,
            condition,
            year,
            offline_mode: true
          }
        ...
        throw new Error(error || 'Failed to create book copies');
      }
    },
    onSuccess: (data) => {
    ...
    onError: (error) => {
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to create book copies',
        variant: 'destructive',
      });
    },
  });
