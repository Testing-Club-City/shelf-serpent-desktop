...
// Default empty fines for instant loading
const defaultFines: any[] = [];

export const useFines = () => {
  return useQuery({
    queryKey: ['fines'],
    queryFn: async () => {
      console.log('⚡ Ultra-fast fines fetch...');
      
      try {
        // Super aggressive timeout for speed
        const controller = new AbortController();
        const timeoutId = setTimeout(() => {
          console.log('⚡ Fines timeout - using defaults');
          controller.abort();
        }, 800); // 800ms timeout
        
        // Simplified query without complex joins for speed
        ...
        if (error) {
          console.log('⚡ Fines error - using defaults');
          return defaultFines;
        }

        console.log('⚡ Fines fetched successfully (simplified)');
        return data || defaultFines;
      } catch (error) {
        console.log('⚡ Fines fetch failed - using defaults (this is fine)');
        return defaultFines;
      }
    },
    // Ultra-fast loading options
    placeholderData: defaultFines, // Show empty list immediately
