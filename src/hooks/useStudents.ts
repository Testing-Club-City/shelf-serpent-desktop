...
// Function to fetch all students with pagination - matches query_students.js approach
async function fetchAllStudents(searchTerm = '', classId?: string) {
  const limit = 1000; // Reduced limit for better performance
  let offset = 0;
  let allStudents: any[] = [];
  let hasMore = true;
  let totalCount = 0;
  ...
  while (hasMore) {
    // Start building the query with proper class relationship like query_students.js
    let query = supabase
      .from('students')
      .select(`
        *,
  ...
  console.log(`Fetched ${allStudents.length} students with class information`);
...
export function useStudents(options: UseStudentsOptions = {}) {
  ...
  return useQuery({
    queryKey: ['students', page, pageSize, searchTerm, classId, fetchAll, 'offline-first'],
    queryFn: async () => {
      try {
        // Try offline first
        ...
        if (connectivity.isOnline) {
          ...
          fetchAllStudents(searchTerm, classId).catch(console.error);
        }
        
        // Apply client-side filtering and pagination for offline data
