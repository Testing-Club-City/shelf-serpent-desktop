...
export function ConnectionStatus({ className, showDetails = true }: ConnectionStatusProps) {
  ...
  const loadStatus = async () => {
    try {
      ...
      if (hasTauri) {
        ...
        try {
          // Get sync status and local data count, use connectivity hook for online status
          ...
          // Use connectivity hook for more reliable online status
          const updatedStatus = {
            ...statusData,
            is_online: connectivity.isOnline
          };
          ...
          console.warn('Tauri commands failed, using connectivity hook fallback:', invokeError);
          setStatus(prev => ({ ...prev, is_online: connectivity.isOnline }));
          setLocalData({ books_count: 0, students_count: 0, categories_count: 0, has_data: false });
        }
      } else {
        // Fallback to connectivity hook
        setStatus(prev => ({ ...prev, is_online: connectivity.isOnline }));
        setLocalData({ books_count: 0, students_count: 0, categories_count: 0, has_data: false });
      }
      setIsLoading(false);
      ...
      // Final fallback to connectivity hook
      setStatus(prev => ({ ...prev, is_online: connectivity.isOnline }));
      setIsLoading(false);
    }
  };
  ...
  const handleCheckConnectivity = async () => {
    try {
      setIsLoading(true);
      // Use the connectivity hook's refresh function for consistency
  ...
  useEffect(() => {
    ...
    // Poll for status updates less frequently since connectivity hook handles real-time updates
    const interval = setInterval(loadStatus, 30000); // Reduced from 5s to 30s
        ...
        {status.last_sync && (
          <div className="flex items-center gap-1 col-span-2">
            ...
            <span className="text-muted-foreground">
              Last sync: {status.last_sync ? new Date(status.last_sync).toLocaleTimeString() : 'N/A'}
            </span>
          </div>
        )}
      </div>
