...
import { ArrowLeft, User, Phone, Mail, Building, Briefcase, Calendar, BookOpen, AlertTriangle, RefreshCw } from 'lucide-react';
...
export const StaffDetails: React.FC<StaffDetailsProps> = ({ staff, onBack }) => {
  ...
  // Get staff borrowing history from local database
  ...
  // Get staff fines from local database
  const { data: fines, isLoading: finesLoading, refetch: refetchFines } = useQuery({
    queryKey: ['staff-fines-local', staff.id],
    queryFn: async () => {
      try {
        // Use Tauri command to get staff fines from local database
        const fines = await invoke('get_fines_by_staff', { staffId: staff.id });
        return fines;
      } catch (error) {
        console.error('Failed to get staff fines:', error);
        return []; // Return empty array if no fines found
      }
    },
  });
  ...
  const handleRefresh = async () => {
    await queryClient.invalidateQueries({ queryKey: ['staff-borrowings-local', staff.id] });
    await queryClient.invalidateQueries({ queryKey: ['staff-fines-local', staff.id] });
