...
export const ReportPreview: React.FC<ReportPreviewProps> = ({ 
  ...
  const generatePreviewData = () => {
    ...
    switch (reportType) {
      ...
      case 'staff_borrowing_history':
        const flattenedStaffHistory = data.staffBorrowingHistory 
          ? Object.values(data.staffBorrowingHistory).flatMap((staffData: any) => 
              staffData.borrowingHistory.map((borrowing: any) => ({
                ...borrowing,
          ...
          : [];
        return {
          headers: ['Staff Name', 'Department', 'Book Title', 'Legacy Book ID', 'Borrowed Date', 'Due Date', 'Status'],
          rows: flattenedStaffHistory.slice(0, 10).map((b: any) => [
            b.staff ? `${b.staff.first_name} ${b.staff.last_name}` : 'Unknown Staff',
  ...
  const generateSummaryStats = () => {
    ...
    switch (reportType) {
      ...
      case 'staff_borrowing_history':
        const staffCount = data.staffBorrowingHistory ? Object.keys(data.staffBorrowingHistory).length : 0;
        const totalBorrowings = data.staffBorrowingHistory 
          ? Object.values(data.staffBorrowingHistory).reduce((acc: number, staff: any) => acc + staff.borrowingHistory.length, 0)
          : 0;
        const activeBorrowings = data.staffBorrowingHistory 
          ? Object.values(data.staffBorrowingHistory).reduce((acc: number, staff: any) => 
              acc + staff.borrowingHistory.filter((b: any) => b.status === 'active').length, 0)
          : 0;
        return [
          { label: 'Staff Members', value: staffCount, icon: Users },
          { label: 'Total Borrowings', value: totalBorrowings, icon: BookOpen },
          { label: 'Active Borrowings', value: activeBorrowings, icon: TrendingUp }
        ];
  ...
  const generateInsights = () => {
    ...
    switch (reportType) {
      ...
      case 'popular_books':
        ...
        const totalBorrowCount = popularBooks.reduce((sum: number, book: any) => sum + (book.borrowCount || 0), 0);
        
        return [
          `⭐ "${mostPopular?.book?.title || 'Unknown'}" is the most borrowed book with ${mostPopular?.borrowCount || 0} borrowings`,
          `📚 Top 10 books account for ${totalBorrowCount} total borrowings`,
          `📊 Average borrowings per popular book: ${Math.round(totalBorrowCount / popularBooks.length)}`,
      ...
      case 'staff_borrowing_history':
        const staffCount = data.staffBorrowingHistory ? Object.keys(data.staffBorrowingHistory).length : 0;
        const totalStaffBorrowings = data.staffBorrowingHistory 
          ? Object.values(data.staffBorrowingHistory).reduce((acc: number, staff: any) => acc + (staff.borrowingHistory?.length || 0), 0)
          : 0;
        const avgPerStaff = staffCount > 0 ? Number(totalStaffBorrowings) / Number(staffCount) : 0;
        
        return [
          `👨🏫 ${staffCount} staff members have borrowing records`,
          `📚 Total of ${totalStaffBorrowings} staff borrowings recorded`,
          `📊 Average ${avgPerStaff.toFixed(1)} books borrowed per staff member`,
          avgPerStaff > 3 ? '✅ Good staff engagement with library resources' : '💡 Encourage staff to utilize library resources more',
          '📈 Staff borrowing patterns can inform professional development resource needs'
        ];
