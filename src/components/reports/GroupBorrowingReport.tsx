...
export const GroupBorrowingReport: React.FC<GroupBorrowingReportProps> = ({ selectedClass = 'all' }) => {
  const { data: groupBorrowings, isLoading: isLoadingBorrowings } = useGroupBorrowingsOffline();
  ...
  const filteredBorrowings = selectedClass === 'all'
    ...
    : (groupBorrowings || []).filter(borrowing => {
        ...
        if (borrowing.student_ids) {
          if (typeof borrowing.student_ids === 'string') {
            try {
              parsedStudentIds = JSON.parse(borrowing.student_ids);
            } catch {
              parsedStudentIds = (borrowing.student_ids as string).split(',').map(id => id.trim());
            }
          } else if (Array.isArray(borrowing.student_ids)) {
            parsedStudentIds = borrowing.student_ids;
          }
        }
        
        return parsedStudentIds.some(id => {
  ...
  const totalStudentsInvolved = new Set(
    filteredBorrowings.flatMap(b => {
      // Parse student IDs from JSON string if needed
      if (b.student_ids) {
        if (typeof b.student_ids === 'string') {
          try {
            return JSON.parse(b.student_ids);
          } catch {
            return (b.student_ids as string).split(',').map(id => id.trim());
          }
        }
        return Array.isArray(b.student_ids) ? b.student_ids : [];
      }
      return [];
    })
  ).size;
  ...
  const getStudentDetails = (studentIds: string | string[]) => {
    // Parse student IDs if they're in JSON string format
    let parsedStudentIds: string[] = [];
    ...
    return parsedStudentIds.map(id => students?.find(s => s.id === id))
                  ...
                  <AccordionContent>
                    <div className="pl-4 pt-4">
                      <div className="space-y-4">
                        ...
                        {/* Group Members */}
                        <div>
                          <div className="font-medium mb-2">Group Members ({groupMembers.length})</div>
