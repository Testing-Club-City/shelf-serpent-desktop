...
export const BulkStudentEntry = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Look for header row containing common student data fields
        for (let i = 0; i < Math.min(5, jsonData.length); i++) {
          const row = jsonData[i];
          if (row && Array.isArray(row)) {
            const rowStr = row.join('|').toLowerCase();
            if (rowStr.includes('name') || rowStr.includes('admission') || rowStr.includes('class') || rowStr.includes('form')) {
              headerRowIndex = i;
        ...
        // If no headers found, assume first row is data and use default headers
        if (headerRowIndex === -1) {
          headerRowIndex = 0;
          headers = ['admission_number', 'name', 'class'];
        }
        ...
        for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
          ...
          for (let j = 0; j < headers.length && j < row.length; j++) {
            ...
            if (header.includes('admission') || header.includes('number') || header.includes('id')) {
              ...
              lastName = value;
            } else if (header.includes('class') || header.includes('form') || header.includes('grade')) {
              className = value;
            }
          }
          
          // If no specific admission number found, use first column
          if (!admissionNumber && row.length > 0) {
            admissionNumber = String(row[0] || '').trim();
          }
          
          // Handle name splitting like in our Python script
          if (fullName && !firstName && !lastName) {
            ...
            if (nameParts.length >= 2) {
              ...
              firstName = nameParts[0];
              lastName = nameParts[0]; // Use same name for both if only one word
            }
          }
          
          // If no className found in specific column, check later columns
          if (!className && row.length > 2) {
            for (let j = 2; j < row.length; j++) {
              const value = String(row[j] || '').trim();
              if (value && (value.match(/^F\d/) || value.toLowerCase().includes('form') || value.toLowerCase().includes('grade'))) {
                className = value;
                break;
              }
            }
          ...
          // Convert F2 -> Form 2, F3 -> Form 3, etc. like in our Python script
          if (className) {
            const formMatch = className.match(/^F(\d+)$/i);
            if (formMatch) {
              className = `Form ${formMatch[1]}`;
            }
          }
          
          // Skip if essential data is missing
          if (!admissionNumber || !firstName || !className) {
            console.warn(`Skipping row ${i + 1}: Missing essential data`, { admissionNumber, firstName, lastName, className });
            continue;
          }
          
          // Create CSV line in our expected format
          const csvLine = [
            admissionNumber,
            firstName,
            lastName || firstName, // Use firstName as lastName if no lastName
            className,
            'form', // academic_system
            className.match(/(\d+)/)?.[1] || '1', // form_level
            'A' // default section
          ].join(',');
        ...
        toast({
          title: 'File Uploaded',
          description: `Successfully processed ${processedData.length} student records from Excel file`,
        });
  ...
  // Download simple Excel template that matches successful import format
  const downloadSimpleExcelTemplate = () => {
    // Create sample data similar to the successful ALL STUDENTS DATA.xlsx format
    const sampleData = [
      ['Admission Number', 'Full Name', 'Class'],
      ['STU001', 'John Doe', 'F2'],
      ['STU002', 'Jane Smith', 'F3'],
      ['STU003', 'Robert Brown', 'F4'],
      ['STU004', 'Alice Johnson', 'F2']
    ];
    ...
    // Generate Excel file and trigger download
    XLSX.writeFile(workbook, 'simple_student_template.xlsx');
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-gray-600">Import multiple students from Excel/CSV files with intelligent parsing, automatic name splitting, and class creation</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={downloadSimpleExcelTemplate} variant="outline">
              ...
              <div>
                ...
                <div className="mt-2">
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Supports .xlsx, .xls, and .csv files. Smart parsing handles various formats:
                    <br />• Full names (automatically split into first/last name)
                    <br />• Class codes like F2, F3, F4 (converted to Form 2, Form 3, Form 4)
                    <br />• Flexible column order with header detection
                  </p>
                </div>
              </div>
            )}
