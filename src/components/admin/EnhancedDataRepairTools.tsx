...
export const EnhancedDataRepairTools: React.FC = () => {
  ...
  const fixAllIssues = async () => {
    ...
    const timeoutId = setTimeout(() => {
      console.error('Repair process timed out after 10 minutes');
      toast({
        title: 'Repair Timeout',
        description: 'Repair process was stopped after 10 minutes to prevent system issues',
        variant: 'destructive',
      });
    ...
    try {
      ...
      if (allBooks) {
        console.log(`Processing ${allBooks.length} books for missing copies...`);
        
        for (const book of allBooks) {
          try {
            processedBooks++;
            
            // Add progress updates every 10 books
            if (processedBooks % 10 === 0) {
              console.log(`Processed ${processedBooks}/${allBooks.length} books...`);
              setRepairProgress(((currentStep - 1) / totalSteps + (processedBooks / allBooks.length) / totalSteps) * 100);
            }
            
            // Skip books without book codes
            if (!book.book_code) {
              console.log(`Skipping book "${book.title}" - no book code`);
              continue;
            }
            ...
            if (fetchError) {
              console.error(`Error fetching copies for book "${book.title}":`, fetchError);
              continue;
            }
            ...
            // Safety check: Don't create more than 50 copies at once
            if (neededCopies > 50) {
              console.warn(`Book "${book.title}" needs ${neededCopies} copies - this seems excessive. Skipping for safety.`);
              continue;
            }
            
            // Safety check: Don't create copies if total_copies is unreasonably high
            if ((book.total_copies || 0) > 1000) {
              console.warn(`Book "${book.title}" has total_copies of ${book.total_copies} - this seems excessive. Skipping for safety.`);
              continue;
            }
          
            if (neededCopies > 0) {
              // Fix: Get the actual copy numbers, not just map to 1
              const existingCopyNumbers = existingCopies?.map(c => c.copy_number || 0) || [0];
              const nextCopyNumber = Math.max(...existingCopyNumbers) + 1;
              
              console.log(`Creating ${neededCopies} copies for book "${book.title}", starting from copy number ${nextCopyNumber}`);
              
              for (let i = 0; i < neededCopies; i++) {
                const copyNumber = nextCopyNumber + i;
                const trackingCode = `${book.book_code}/${String(copyNumber).padStart(3, '0')}/24`;
                
                console.log(`Creating copy ${copyNumber} with tracking code: ${trackingCode}`);
                ...
                if (error) {
                  console.error(`Error creating copy ${copyNumber} for book "${book.title}":`, error);
                } else {
                  createdCopies++;
                  console.log(`✅ Successfully created copy ${copyNumber}`);
                }
                
                // Add small delay between copy creations to prevent overwhelming the system
                if (i < neededCopies - 1) {
            ...
            // Add delay between books to prevent system overload
            if (processedBooks < allBooks.length) {
