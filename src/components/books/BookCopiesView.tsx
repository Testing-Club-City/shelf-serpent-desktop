...
import { useLocalBookCopies } from '@/hooks/useLocalBookCopies';
...
const BookCopiesView: React.FC<BookCopiesViewProps> = ({ 
  ...
  // Use local database hook instead of network-based hook
  const { 
    data: bookCopiesData, 
    isLoading, 
    error, 
    refetch 
  } = useLocalBookCopies(bookId);
  ...
  // Apply client-side pagination to local data
  const startIndex = (currentPage - 1) * pageSize;
  const paginatedCopies = bookCopies.slice(startIndex, startIndex + pageSize);
  ...
  const bookCode = bookCopies?.[0]?.book_code || '';
  ...
  useEffect(() => {
    const fetchHighestCopyNumber = async () => {
      try {
        if (bookId) {
          const highest = await invoke('get_highest_copy_number', { bookId });
          setHighestCopyNumber(highest || 0);
        }
      } catch (error) {
        console.error('Error fetching highest copy number:', error);
        setHighestCopyNumber(0);
      }
    };
  ...
  const handleAddCopies = async () => {
    ...
    try {
      ...
      // Refetch the book copies data
      await refetch();
      
      // Also refresh the highest copy number to ensure accuracy
      try {
        const refreshedHighest = await invoke('get_highest_copy_number', { bookId });
        setHighestCopyNumber(refreshedHighest || 0);
      } catch (error) {
        console.error('Error refreshing highest copy number:', error);
      }
    } catch (error) {
  ...
  const displayTrackingCode = (copy: any) => {
    ...
    const legacyId = String(copy.legacy_book_id || 'UNK').padStart(3, '0');
    const year = new Date().getFullYear().toString().slice(-2);
    return `${bookCode}/${legacyId}/${year}`;
  };
