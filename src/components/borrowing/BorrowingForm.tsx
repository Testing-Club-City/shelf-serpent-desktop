...
export const BorrowingForm: React.FC<BorrowingFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate student status for borrowing
    if (borrowerType === 'student' && selectedBorrower.status !== 'active') {
      console.error('Inactive student cannot borrow:', selectedBorrower);
      const statusText = selectedBorrower.status === 'graduated' ? 'graduated' : 
                        selectedBorrower.status === 'inactive' ? 'inactive' : 
                        selectedBorrower.status || 'inactive';
      alert(`Cannot issue book to ${selectedBorrower.first_name} ${selectedBorrower.last_name}. Student is ${statusText} and cannot borrow books.`);
      return;
    }
