...
import { useCreateBorrowingOffline } from '@/hooks/useBorrowingsOffline';
...
export const ImprovedTrackingCodeBorrowingForm: React.FC<ImprovedTrackingCodeBorrowingFormProps> = ({
  ...
  const createBorrowing = useCreateBorrowingOffline();
    ...
    try {
      ...
      for (const borrowing of validBorrowings) {
        const borrowingData = {
          ...
          borrow_date: format(new Date(), 'yyyy-MM-dd'), // Changed from borrowed_date to borrow_date
