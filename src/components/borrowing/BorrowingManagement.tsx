...
import { useBorrowingsOffline, useCreateBorrowingOffline } from '@/hooks/useBorrowingsOffline';
import { useBorrowingsEnhanced, useOverdueBorrowingsEnhanced, useActiveBorrowingsEnhanced, useReturnedBorrowingsEnhanced, useCreateBorrowingEnhanced, useReturnBookEnhanced } from '@/hooks/useBorrowingsEnhanced';
...
const safeFormat = (dateValue: string | Date | null | undefined, formatPattern: string, fallback: string = 'N/A'): string => {
...
import { useGroupBorrowingsOffline, useCreateGroupBorrowingOffline, useReturnGroupBorrowingOffline } from '@/hooks/useGroupBorrowingsOffline';
...
export const BorrowingManagement = ({ initialTab = 'overview' }: BorrowingManagementProps) => {
  ...
  const [showEnhancedReturnForm, setShowEnhancedReturnForm] = useState(false);
  ...
  const [showGroupStudentsDialog, setShowGroupStudentsDialog] = useState(false);
  const [selectedGroupBorrowing, setSelectedGroupBorrowing] = useState<any>(null);
  ...
  const { data: staffSearchResults, isLoading: staffSearchLoading } = useQuery({
    queryKey: ['staff-borrowing-search', staffSearchTerm],
    queryFn: async () => {
      if (!staffSearchTerm.trim()) return [];
  ...
  const [fastBorrowings, setFastBorrowings] = useState<any[]>([]);
    ...
    const fetchFastData = async () => {
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Borrowings query timeout')), 3000)
        );
        
        const borrowingsPromise = invoke('get_borrowings');
        
        const directBorrowings = await Promise.race([borrowingsPromise, timeoutPromise]);
        
        if (isMounted) {
          setFastBorrowings(Array.isArray(directBorrowings) ? directBorrowings : []);
          setFastLoading(false);
        }
      } catch (error) {
        console.warn('Fast borrowings load failed:', error);
        if (isMounted) {
          setFastBorrowings([]);
          setFastLoading(false);
        }
  ...
  // Use fast offline-first hooks only
  const { data: borrowingsOffline, isLoading: offlineLoading } = useBorrowingsOffline();
  
  // Memoized data processing for performance  
  const borrowings = useMemo(() => {
    // Use borrowingsOffline if available (has complete data), otherwise fallback to fastBorrowings
    const data = borrowingsOffline || fastBorrowings || [];
    console.log('📊 Using borrowings data:', data.length, 'records');
    if (data.length > 0) {
      console.log('📋 Sample borrowing:', data[0]);
    }
    return data;
  }, [borrowingsOffline, fastBorrowings]);
  
  const { activeBorrowings, overdueBorrowings, recentReturns } = useMemo(() => {
    const now = getSafeCurrentDate();
    const active = [];
    const overdue = [];
    const returned = [];
    
    for (const b of borrowings) {
      if (b.status === 'returned' || b.returned_date) {
        returned.push(b);
      } else if ((b.status === 'active' || b.status === 'borrowed') && !b.returned_date) {
        active.push(b);
        // Only consider short-term borrowings as overdue for fine purposes
        if (b.due_date && new Date(b.due_date) < now && !b.is_long_term) {
          overdue.push(b);
        }
      }
    }
    
    // Sort returned books by latest return date first
    const sortedReturns = returned.sort((a, b) => {
      const aReturnDate = new Date(a.returned_date || a.updated_at || '1970-01-01');
      const bReturnDate = new Date(b.returned_date || b.updated_at || '1970-01-01');
      return bReturnDate.getTime() - aReturnDate.getTime(); // Latest first
    });
    
    return {
      activeBorrowings: active,
      overdueBorrowings: overdue,
      recentReturns: sortedReturns
    };
  }, [borrowings]);
  const { data: fineCollection, isLoading: finesLoading } = useFineCollection();
  const { data: allFines } = useFines();
  // Use offline-first hooks for group borrowings
  const { data: groupBorrowings, isLoading: groupBorrowingsLoading } = useGroupBorrowingsOffline();
  const payFine = usePayFine();
  const clearFine = useClearFine();
  const collectFine = useCollectFine();
  const returnBorrowing = useReturnBookEnhanced(); // Use enhanced return hook
  const createBorrowing = useCreateBorrowingEnhanced(); // Use enhanced create hook
  const createMultipleBorrowings = useCreateMultipleEnhancedBorrowings(); // Use enhanced multiple create hook
  const createGroupBorrowing = useCreateGroupBorrowingOffline();
  const returnGroupBorrowing = useReturnGroupBorrowingOffline();
  const { updatePageState } = useDocumentMetaContext?.() ?? { updatePageState: () => {} };
  ...
  const isLoading = offlineLoading || fastLoading;

  // Optimized page state updates
  useEffect(() => {
    if (isLoading) {
      updatePageState('loading');
    } else {
      const titles = {
        overview: 'Borrowing Management Overview',
        active: 'Active Student Borrowings',
        staff: 'Active Staff Borrowings',
        groups: 'Group Borrowings',
        overdue: 'Overdue Books',
        returns: 'Recent Returns',
        fines: 'Fines Management'
      };
      updatePageState('success', titles[activeTab] || 'Borrowing Management');
    }
  }, [isLoading, activeTab, updatePageState]);
  const totalFines = fineCollection?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
  ...
  const filteredActiveBorrowings = useMemo(() => {
    let filtered = activeBorrowings;
    
    // Filter by search query if provided
    if (searchQuery) {
      const search = searchQuery.toLowerCase();
      filtered = activeBorrowings.filter(borrowing => {
        const student = (borrowing as any).students;
        const book = (borrowing as any).books;
        
        return (student?.admission_number?.toLowerCase().includes(search)) ||
               (`${student?.first_name} ${student?.last_name}`.toLowerCase().includes(search)) ||
               (book?.title?.toLowerCase().includes(search)) ||
               (student?.class_grade?.toLowerCase().includes(search));
      });
    }
    
    // Sort by latest borrowings first (most recent borrowed_date first)
    return filtered.sort((a, b) => {
      if (searchQuery) {
        // If searching, prioritize admission number matches first
        const search = searchQuery.toLowerCase();
        const aMatch = ((a as any).students?.admission_number?.toLowerCase() || '').includes(search);
        const bMatch = ((b as any).students?.admission_number?.toLowerCase() || '').includes(search);
        if (aMatch && !bMatch) return -1;
        if (!aMatch && bMatch) return 1;
      }
      
      // Sort by latest borrowings first - use multiple date fields for robustness
      const aDate = new Date(a.borrowed_date || a.created_at || '1970-01-01');
      const bDate = new Date(b.borrowed_date || b.created_at || '1970-01-01');
      
      // Most recent first (DESC order)
      return bDate.getTime() - aDate.getTime();
    });
  }, [activeBorrowings, searchQuery]);
  ...
  const paginatedData = useMemo(() => {
    ...
    if (staffSearchTerm.trim()) {
      staffBorrowings = staffSearchResults || [];
    } else {
  ...
  const getStatusBadge = (dueDate: string, isLongTerm?: boolean) => {
      ...
      if (daysOverdue > 0) {
        ...
        return (
          <Badge variant="secondary" className="bg-blue-100 text-blue-700">
            Long-term ({daysUntilDue} days left)
          </Badge>
        );
      }
    }
  ...
  const handleReturnSubmit = async (returnData: any) => {
    ...
    try {
      ...
      if (returnData.is_theft) {
        console.log('Processing theft case...');

        // Create a fine for the thief
        if (returnData.thief_student_id) {
          const fineAmount = returnData.fine_amount || 800;
          
          await supabase.from('fines').insert({
            student_id: returnData.thief_student_id,
            amount: fineAmount,
            fine_type: 'theft',
            description: `Theft fine for book. Tracking code: ${returnData.returned_tracking_code}`,
            status: 'unpaid',
            created_at: getSafeISOString(),
            related_borrowing_id: returnData.borrowing_id
          });
          
          console.log(`Created theft fine for student ${returnData.thief_student_id}`);
        }
        
        // Return the book to the victim's account - call backend directly
        await invoke('return_book', {
          borrowingId: returnData.borrowing_id,
          returnData: {
            ...
            copy_condition: returnData.condition_at_return,
            is_lost: false
          }
        });
      } else {
        // Normal return process - call backend directly with correct parameters
        await invoke('return_book', {
          borrowingId: returnData.borrowing_id,
          returnData: {
            returned_date: returnData.returned_date,
            ...
            copy_condition: returnData.condition_at_return,
            is_lost: returnData.is_lost || false
          }
        });
      }

      // Refresh data - invalidate all relevant query keys
        ...
        switch (action) {
          ...
          case 'collect':
            ...
            if (amount && amount > 0) {
              ...
              } else if (new Date(borrowing.due_date) < new Date(borrowing.returned_date || getSafeISOString())) {
  ...
  const handleBorrowingSubmit = async (borrowings: any[]) => {
    // Pause connectivity checks during processing
    console.log('🔇 Pausing connectivity checks during enhanced borrowing submission');
    connectivity.pauseChecks();
    
    try {
      console.log('📚 Processing enhanced borrowing submission:', borrowings.length, 'items');
      
      // Use enhanced multiple borrowings creation for better error handling
      await createMultipleBorrowings.mutateAsync(borrowings);
      
      console.log('✅ Enhanced borrowing submission completed successfully');
      setShowBorrowingForm(false);
    } catch (error) {
      console.error('❌ Enhanced borrowing submission failed:', error);
      // Error handling is done by the hook, so we don't need to show additional toasts
    } finally {
      // Always resume connectivity checks
      console.log('🔊 Resuming connectivity checks after enhanced borrowing submission');
  ...
  const handleGroupBorrowingSubmit = async (groupBorrowing: any) => {
    // Pause connectivity checks during processing
    console.log('🔇 Pausing connectivity checks during group borrowing submission');
    connectivity.pauseChecks();
    
    try {
      console.log('📋 Group borrowing data received:', JSON.stringify(groupBorrowing, null, 2));
      ...
      if (!groupBorrowing.book_id || !groupBorrowing.book_copy_id) {
        throw new Error('Missing book information - please search for a book first');
      }
      
      console.log('✅ Validation passed, creating group borrowing...');
      
      // Use mutateAsync for better error handling
      await createGroupBorrowing.mutateAsync(groupBorrowing);
      
      // Refresh the group borrowings data
      queryClient.invalidateQueries({ queryKey: ['group-borrowings', 'offline-first'] });
      queryClient.invalidateQueries({ queryKey: ['group-borrowings'] });
      ...
      // Always resume connectivity checks
      console.log('🔊 Resuming connectivity checks after group borrowing submission');
      connectivity.resumeChecks();
    }
  };
  ...
  const handleGroupReturnSubmit = async (returnData: any) => {
    try {
      ...
      // Process the group return using the offline hook - note the different parameter structure
      await returnGroupBorrowing.mutateAsync({
        groupBorrowingId: returnData.group_borrowing_id,
        returnData: {
          returned_date: getSafeISOString(),
          condition_at_return: returnData.condition_at_return,
          return_notes: returnData.return_notes,
          returned_by: null // You could get the current user ID if needed
        }
      });
  ...
  const renderBorrowingRow = (borrowing: any, index: number) => {
    ...
    return (
      <TableRow key={borrowing.id}>
        ...
        <TableCell>
          {(borrowing as any).borrower_type === 'staff' || (borrowing as any).staff_id ? (
            <div>
        ...
        <TableCell>
          <div>
            <p className="font-medium">Legacy ID: {(borrowing as any).book_copies?.legacy_book_id || 'N/A'}</p>
            <p className="text-xs text-gray-500">by {(borrowing as any).books?.author}</p>
        ...
        <TableCell>
          <div>
            <p className="font-medium">{safeFormat(borrowing.due_date, 'MMM dd, yyyy')}</p>
            <p className="text-xs text-gray-500">
              {safeFormat(borrowing.due_date, 'EEEE')}
            </p>
          </div>
        </TableCell>
        <TableCell>
          {getStatusBadge(borrowing.due_date, borrowing.is_long_term)}
        </TableCell>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Showing {activeGroupBorrowings.length} active group {activeGroupBorrowings.length === 1 ? 'borrowing' : 'borrowings'} (sorted by latest)
            </p>
          </div>
          <Button onClick={() => setShowGroupBorrowingForm(true)}>
            <Plus className="w-4 h-4 mr-2" />
              ...
              <TableRow key={borrowing.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => {
                        setSelectedGroupBorrowing(borrowing);
                        setShowGroupStudentsDialog(true);
                      }}
                    ...
                    >
                      <UsersRound className="h-4 w-4 text-muted-foreground group-hover:text-blue-600 transition-colors" />
                ...
                <TableCell>
                  {safeFormat(borrowing.borrowed_date, 'MMM d, yyyy')}
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
                    {safeFormat(borrowing.due_date, 'MMM d, yyyy')}
                    {getStatusBadge(borrowing.due_date, borrowing.is_long_term)}
                  </div>
                </TableCell>
                <TableCell className="text-right">
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b pb-4">
          <h2 className="text-xl font-semibold">Issue Books</h2>
        </div>
  ...
  if (showEnhancedReturnForm) {
    return (
      <EnhancedBookReturnForm
        onSubmit={handleReturnSubmit}
        onCancel={() => setShowEnhancedReturnForm(false)}
      />
    );
  }
  ...
  if (showReturnForm) {
    if (!selectedBorrowing) {
      // Show DirectReturnForm for manual return processing
      return (
        <DirectReturnForm
          onSubmit={handleReturnSubmit}
  ...
  return (
    <div className="space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <StatCard title="Active Borrowings" value={activeBorrowings.length} total={borrowings.length} icon={Activity} description="Currently borrowed books" />
            <StatCard title="Overdue Books" value={overdueBorrowings.length} total={overdueBorrowings.length} icon={AlertTriangle} description="Books past due date" color="red" />
            <StatCard 
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="relative overflow-hidden">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Found Books</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between">
                  <RotateCcw className="h-12 w-12 text-blue-600" />
                  <Button onClick={() => setShowEnhancedReturnForm(true)} className="bg-green-600 hover:bg-green-700">
                    Found Books
                  </Button>
                </div>
              </CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  paginatedData.activeBorrowings.map((borrowing, idx) => renderBorrowingRow(borrowing, idx))
                )}
              </TableBody>
            </Table>
          </div>
          {!isLoading && renderPagination(activePage, getTotalPages(filteredActiveBorrowings.filter(b => (b as any).borrower_type === 'student' || !(b as any).borrower_type).length), setActivePage)}
        </TabsContent>
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search staff by phone number or TSC number..."
            ...
            {staffSearchTerm && (
              <div className="mt-2 text-sm text-gray-600">
                {staffSearchLoading ? (
                  <span className="flex items-center gap-2">
                    <div className="h-4 w-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                    Searching for: <span className="font-medium">"{staffSearchTerm}"</span>
                  </span>
                ) : (
                  <>
                    Searching for: <span className="font-medium">"{staffSearchTerm}"</span>
                    {paginatedData.staffBorrowings.length > 0 && (
                      <span className="ml-2 text-green-600">
                        ({paginatedData.staffBorrowings.length} result{paginatedData.staffBorrowings.length !== 1 ? 's' : ''} found)
                      </span>
                    )}
                    {paginatedData.staffBorrowings.length === 0 && !staffSearchLoading && (
                      <span className="ml-2 text-red-600">
                        (No results found)
                      </span>
                    )}
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  paginatedData.staffBorrowings.map((borrowing, idx) => renderBorrowingRow(borrowing, idx))
                )}
              </TableBody>
            </Table>
          </div>
          {!isLoading && renderPagination(staffPage, getTotalPages(filteredActiveBorrowings.filter(b => (b as any).borrower_type === 'staff').length), setStaffPage)}
        </TabsContent>
        ...
        <TabsContent value="overdue" className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <LoadingState />
                ) : paginatedData.overdueBorrowings.length === 0 ? (
                  <EmptyState message="No overdue books found" />
                ) : (
                  paginatedData.overdueBorrowings.map((borrowing) => {
          ...
          {!isLoading && renderPagination(overduePage, getTotalPages(overdueBorrowings.length), setOverduePage)}
        </TabsContent>
        ...
        <TabsContent value="returns" className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <LoadingState />
                ) : paginatedData.returns.length === 0 ? (
                  <EmptyState message="No recent returns found" />
                ) : (
                  paginatedData.returns.map((borrowing) => (
                    <TableRow key={borrowing.id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <BookOpen className="h-4 w-4 text-gray-500" />
                          <div>
                            ...
                            <p className="text-xs text-gray-500">by {borrowing.books?.author || 'Unknown Author'}</p>
                            {borrowing.tracking_code && (
                              <p className="text-xs text-gray-500 font-mono bg-gray-100 px-2 py-1 rounded w-fit mt-1">
                                {borrowing.tracking_code}
                              </p>
                            )}
                          </div>
                      ...
                      <TableCell>
                        <div>
                          <p className="font-medium">
                            {borrowing.borrowed_date ? safeFormat(borrowing.borrowed_date, 'MMM dd, yyyy') : 'N/A'}
                          </p>
                          {borrowing.borrowed_date && (
                            <p className="text-xs text-gray-500">
                              {safeFormat(borrowing.borrowed_date, 'HH:mm')}
                            </p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          <p className="font-medium">
                            {borrowing.returned_date ? safeFormat(borrowing.returned_date, 'MMM dd, yyyy') : 'N/A'}
                          </p>
                          {borrowing.returned_date && (
                            <p className="text-xs text-gray-500">
                              {safeFormat(borrowing.returned_date, 'HH:mm')}
                            </p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        {borrowing.condition_at_return ? (
                          <Badge variant={
                            ...
                            {borrowing.condition_at_return.charAt(0).toUpperCase() + borrowing.condition_at_return.slice(1)}
                          </Badge>
                        ) : (
          ...
          {!isLoading && renderPagination(returnsPage, getTotalPages(recentReturns.length), setReturnsPage)}
        </TabsContent>
        ...
        <TabsContent value="fines" className="space-y-6">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {isLoading ? (
                      <LoadingState />
                    ) : paginatedData.fines.length === 0 ? (
                      <EmptyState message="No fines found" />
                    ) : (
                      paginatedData.fines.map((fine: any) => {
                        ...
                        return (
                          <TableRow key={fine.id}>
                            ...
                            <TableCell>
                              <div>
                                <p className="font-medium">{safeFormat(fine.created_at, 'MMM dd, yyyy')}</p>
                                <p className="text-xs text-gray-500">{safeFormat(fine.created_at, 'HH:mm')}</p>
                              </div>
                            </TableCell>
                            <TableCell className="text-right">
              ...
              {!isLoading && renderPagination(finesPage, getTotalPages(combinedFinesData.length), setFinesPage)}
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
      ...
      {selectedGroupBorrowing && (
        <GroupStudentsDialog
          isOpen={showGroupStudentsDialog}
          onClose={() => {
            setShowGroupStudentsDialog(false);
            setSelectedGroupBorrowing(null);
          }}
          studentIds={selectedGroupBorrowing.student_ids || []}
          groupBorrowingId={selectedGroupBorrowing.id}
          bookTitle={selectedGroupBorrowing.books?.title}
        />
      )}
