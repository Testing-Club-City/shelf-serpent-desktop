...
export const GroupBorrowingForm: React.FC<GroupBorrowingFormProps> = ({
  ...
  const handleBorrowedDateChange = (date: string) => {
    ...
    const newDueDate = calculateDueDate(getSafeDate(date));
  ...
  const searchStudentByAdmission = async (admissionNumber: string, index: number) => {
    ...
    try {
      console.log('🔍 Searching for student with admission number:', trimmedAdmission);
      
      // Search for the student using offline-first approach
      ...
      if (student) {
        ...
        const existingIndex = selectedStudents.findIndex(s => s && s.admission_number === trimmedAdmission);
        ...
        // Clear the selected student for this index
        setSelectedStudents(prev => {
          const updated = [...prev];
          updated[index] = null;
          return updated;
  ...
  const handleSearchBookCopy = async () => {
    ...
    try {
          // Search for book copy by legacy book ID
      const legacyBookId = parseInt(trackingCode);
      if (isNaN(legacyBookId)) {
        throw new Error('Invalid book ID format');
      }
      const bookCopy = await invoke('search_book_copy_by_id', {
        bookId: legacyBookId
      });

      if (bookCopy) {
        setBookCopy(bookCopy);
        setConditionAtIssue(bookCopy.condition);
        
        toast({
          title: "Book Copy Found",
          description: `Found: ${bookCopy.title || bookCopy.books?.title || bookCopy.book?.title || 'Unknown Title'} (Copy #${bookCopy.copy_number || 'N/A'})`,
        });
      } else {
        toast({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const groupBorrowing = {
        book_id: bookCopy.book_id || bookCopy.id,
      ...
      console.log('📤 Submitting group borrowing data:', {
        ...groupBorrowing,
        student_count: validStudents.length,
        student_names: validStudents.map(s => `${s.first_name} ${s.last_name}`),
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input
                placeholder="Enter book ID (e.g., 893)"
                value={trackingCode}
              ...
              <div className="p-4 border rounded-lg bg-muted">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <h4 className="font-medium">{bookCopy.title || bookCopy.books?.title || 'Unknown Title'}</h4>
                    <p className="text-sm text-muted-foreground">by {bookCopy.author || bookCopy.books?.author || 'Unknown Author'}</p>
                    <div className="flex gap-2">
