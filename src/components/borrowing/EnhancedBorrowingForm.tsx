...
import { BookWithDetails, StudentWithClass } from '@/types/offline';
import { BookCopy } from '@/lib/database-schema';

// Use the desktop version's types
type Student = StudentWithClass;
type Book = BookWithDetails;
...
export const AdvancedBorrowingForm = ({ onSubmit, onCancel }: AdvancedBorrowingFormProps) => {
  ...
  useDebounce(studentSearchTerm, 300); // For potential future use
  ...
  const maxBooksAllowed = 3; // Default borrowing limit since max_books_allowed doesn't exist in Class type
  ...
  const totalStudentPages = 1; // Offline version doesn't use pagination
  ...
  const filteredBooks = availableBooks.filter(book => {
    ...
    return (
      ...
      (book as any).book_code?.toLowerCase().includes(searchLower)
    );
  });
                      ...
                      <CommandGroup>
                        {students
                          ...
                          .map((student) => (
                            <CommandItem
                              key={student.id}
                              value={`${student.first_name} ${student.last_name} ${student.admission_number || 'N/A'}`}
                              onSelect={() => {
                ...
                <ScrollArea className="max-h-80 border rounded-md p-3 bg-gray-50">
                  <div className="space-y-3">
                    {paginatedSelectedBooks.map((book) => {
                      ...
                      const filteredCopies = getFilteredBookCodes(book.id, availableCopies as any[]);
                      ...
                      // Get filter options
                      const availableYears = getAvailableYears(availableCopies as any[]);
                      const availableConditions = getAvailableConditions(availableCopies as any[]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm"><span className="font-medium">Admission No:</span> {selectedStudent.admission_number || 'N/A'}</p>
