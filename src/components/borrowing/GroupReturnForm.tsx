...
export const GroupReturnForm: React.FC<GroupReturnFormProps> = ({
  ...
  const { data: groupBorrowings } = useGroupBorrowingsOffline();
  ...
  const handleSearchStudent = async () => {
    ...
    try {
      // Find the student in offline data
      ...
      // Find active group borrowings for this student
      const activeGroupBorrowings = groupBorrowings?.filter(gb => {
        if (gb.status !== 'active') return false;
        
        // Parse student IDs from JSON string if needed
        let parsedStudentIds: string[] = [];
        
        if (gb.student_ids) {
          if (typeof gb.student_ids === 'string') {
            try {
              parsedStudentIds = JSON.parse(gb.student_ids);
            } catch {
              parsedStudentIds = (gb.student_ids as string).split(',').map(id => id.trim());
            }
          } else if (Array.isArray(gb.student_ids)) {
            parsedStudentIds = gb.student_ids;
          }
        }
        
        return parsedStudentIds.includes(studentData.id);
      });

      if (!activeGroupBorrowings || activeGroupBorrowings.length === 0) {
        toast({
      ...
      setGroupBorrowing(activeGroupBorrowings[0]);
      toast({
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className="bg-blue-100 text-blue-800 border-blue-300">
                          Copy #{groupBorrowing.book_copies?.copy_identifier || 'N/A'}
                        </Badge>
                        <Badge variant="outline" className="bg-blue-100 text-blue-800 border-blue-300">
