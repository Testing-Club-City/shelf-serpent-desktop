...
export const StudentSelector: React.FC<StudentSelectorProps> = ({
  ...
  // Filter students based on search term and exclude inactive/graduated students
  const filteredStudents = students?.filter(student => {
    ...
    // Only show active students for borrowing
    const isActive = student.status === 'active' || !student.status; // Default to active if status is undefined
  ...
  // Count inactive students that match search (for informational purposes)
  const inactiveMatchingStudents = students?.filter(student => {
    const fullName = `${student.first_name} ${student.last_name}`.toLowerCase();
    const admissionNumber = student.admission_number?.toLowerCase() || '';
    const classGrade = student.class_grade?.toLowerCase() || '';
    const search = searchTerm.toLowerCase();

    const isInactive = student.status === 'inactive' || student.status === 'graduated';
    const matchesSearch = fullName.includes(search) || 
                         admissionNumber.includes(search) ||
                         classGrade.includes(search);
        ...
        <div className="border rounded-lg bg-white shadow-sm">
          {paginatedStudents.length > 0 ? (
            <>
              ...
              {/* Show warning if inactive students match search */}
              {inactiveMatchingStudents.length > 0 && (
                <div className="p-3 border-t bg-amber-50 border-amber-200">
                  <div className="flex items-center gap-2 text-amber-800">
                    <AlertCircle className="w-4 h-4" />
                    <span className="text-sm">
                      {inactiveMatchingStudents.length} inactive/graduated student{inactiveMatchingStudents.length > 1 ? 's' : ''} found but hidden. 
                      Inactive students cannot borrow books.
                    </span>
                  </div>
                </div>
              )}
